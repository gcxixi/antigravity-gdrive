...
### 准则 4：沉淀文档强制 Markdown 格式（Strict Markdown Only）
- 凡是用户要求撰写文档、架构设计、技术对比等并存入 Google Drive 的任务，**文件格式必须严格限定为 Markdown（`.md`）**。
- 严禁输出为 `.docx`、`.txt`、`.pdf` 等封闭或二进制格式，保证云端资产的纯文本轻量化、版本可控与跨平台可检索。
- 命名统一遵守全小写中划线命名（`kebab-case`）。
